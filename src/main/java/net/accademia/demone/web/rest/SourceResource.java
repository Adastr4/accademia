...
/**
 * REST controller for managing {@link net.accademia.demone.domain.Source}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class SourceResource {
    ...
    public ResponseEntity<Source> updateSource(@PathVariable(value = "id", required = false) final Long id, @RequestBody Source source)
        ...
        Optional<Source> result = sourceRepository
            ...
            .map(existingSource -> {
                if (source.getSourceid() != null) {
                    existingSource.setSourceid(source.getSourceid());
                }
                if (source.getDescription() != null) {
                    existingSource.setDescription(source.getDescription());
                }
                if (source.getFonte() != null) {
                    existingSource.setFonte(source.getFonte());
                }
                if (source.getData() != null) {
                    existingSource.setData(source.getData());
                }
    ...
    public List<Source> searchSources(@RequestParam String query) {
        log.debug("REST request to search Sources for query {}", query);
        return StreamSupport.stream(sourceSearchRepository.search(query).spliterator(), false).collect(Collectors.toList());
    }
}
