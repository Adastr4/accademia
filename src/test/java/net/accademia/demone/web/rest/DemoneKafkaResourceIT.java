...
import net.accademia.demone.config.KafkaProperties;
...
class DemoneKafkaResourceIT {
    ...
    void setup() {
        ...
        DemoneKafkaResource kafkaResource = new DemoneKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/demone-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/demone-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
